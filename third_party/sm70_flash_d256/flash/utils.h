...
__forceinline__ __device__ auto convert_layout_acc_rowcol(Layout acc_layout) {
    ...
    // Strides:           (1, 2, 4)
    // The column dimension should be the three flattened dimensions (Frag0, Frag2, MMA_N).
    // Wrapping every argument in a single make_layout flattens them.
    return make_layout(
        ...
        // Fix: do not nest make_layout; pass the arguments flat.
...
__forceinline__ __device__ auto convert_layout_C_to_A_v2(
    ...
    for (int j = 0; j < size(tOrP_copy_view); ++j) {
        ...
        if constexpr (sizeof(Element) == 2) {
            ...
            // Pack the registers with plain CUDA.
            uint32_t packed;
...
__forceinline__ __device__ auto convert_layout_acc_dropout(Layout acc_layout) {
    ...
    // After Q.K^T on V100 each thread holds exactly 8 elements,
    // which matches the Philox engine's 8 fp16 (128-bit) per call without borrowing from MMA_N.
    return acc_layout;
};
...
// NOTE: the upstream paged-KV helper `resolve_thread_kv_page_slice_offset`
// was dropped during the llama.cpp port: it is only reachable from the
// PagedKV=true template path (never instantiated here) and is the sole
// user of <optional> in this header. Dropping the dead helper is the
// minimal fix (no <optional> include added, no behavior change).
