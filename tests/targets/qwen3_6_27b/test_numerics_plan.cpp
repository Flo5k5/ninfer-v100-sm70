...
//
// The FP32 residual stream is accepted only for qwen3.8-27b/nvfp4, nvfp4-full-a, nvfp4-full-b and
// nvfp4-full-c without DFlash, and unknown option values are rejected. The residual is the root
// allocation of the prefill chunk, the ordinary decode batch (TextContext::ordinary_decode_batch)
// and the MTP lookup verify aggregate (TextContext::target_verify_batch_impl), so planning it in
// FP32 must grow each of these phases by exactly two more bytes per element on the four profiles.
// The prefill plan must also reserve the staging of the selected attention kernel.
...
constexpr std::array kFp32ResidualProfiles = {
    ...
    std::pair{WeightsProfile::Qwen38Nvfp4FullC, "qwen3.8-27b/nvfp4-full-c"},
};
