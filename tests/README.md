...
The derived Qwen3.8-27B NVFP4 profiles (`nvfp4-full-a`, `nvfp4-full-b`, `nvfp4-full-c`) have
load-plan tests, registered on sm_70 builds only. They bind the artifact on the host without GPU
work, although the binary needs the CUDA driver library to load. They check:

- the identity and the format of every role;
- for `nvfp4-full-b` and `nvfp4-full-c`, the rows and the activation input divisors of the NVFP4
  attention and GDN input leaves;
- for `nvfp4-full-c`, the same of the NVFP4 attention and GDN output projections, each one leaf
  covering its whole object;
...
full-a for full-b, full-b for full-c.
...
NINFER_QWEN3_8_27B_NVFP4_FULL_C_WEIGHTS=$PWD/out/qwen3_8_27b_nvfp4_full_c.ninfer \
NINFER_QWEN3_8_27B_NVFP4_FULL_B_WEIGHTS=$PWD/out/qwen3_8_27b_nvfp4_full_b.ninfer \
  ctest --test-dir build -R ninfer_qwen3_8_27b_nvfp4_full_c_load_plan_test --output-on-failure
```

`NINFER_QWEN3_8_27B_NVFP4_WEIGHTS` has two meanings:

- the artifact under test for the FP32 residual real tests (`nvfp4`, `nvfp4-full-a`,
  `nvfp4-full-b` or `nvfp4-full-c`);
