...
int run_fp32_residual_shape(std::int32_t n, std::int32_t k, std::uint32_t seed) {
    ...
    for (const std::int32_t tokens : kTokens) {
        ...
        const std::size_t capacity = ops::linear_add_workspace_capacity_bytes(
            QType::FP8_E4M3FN_ROW_BF16S, n, k, ops::LinearPolicy::A16Only, tokens, tokens);
        WorkspaceArena workspace(std::max<std::size_t>(capacity, 256));
        ...
        const std::string label = std::string("FP8") + " linear_add FP32 residual [" +
...
#ifdef NINFER_VOLTA_BUILD
    failures += run_fp32_residual_shape(5120, 6144, 865U);
    failures += run_fp32_residual_shape(5120, 17408, 867U);
#endif
