...
int run_nvfp4_a16() {
    ...
    // form (no workspace) is the one the target uses. The policy form at T=64 has a workspace:
    // on the checkpoint-native layout it takes the wide-T MMA route, on the prepacked layout the
    // layout guard must keep it on QPN2.
    ...
    // Production layout: the runtime QPN-prepacks the head at load, so these run the prepacked
    // QPN2 kernel with the generic schedules.
    failures += run_shape("NVFP4_A16 prepacked", ActivationCompute::A16, make_nvfp4_weight,
                          {248320, 5120, 709U, Comparison::Sampled, true, vocabulary_invocations,
                           /*prepack_nvfp4_for_qpn=*/true});
    // The same layout guard for a registered shape reached through linear() with a workspace.
    constexpr std::array prepacked_wide_invocations{
        Invocation{1, CallForm::A16Convenience},
        Invocation{64, CallForm::Policy, ops::LinearPolicy::A16Only},
    };
    failures += run_shape("NVFP4_A16 prepacked", ActivationCompute::A16, make_nvfp4_weight,
                          {5120, 17408, 711U, Comparison::Sampled, true, prepacked_wide_invocations,
                           /*prepack_nvfp4_for_qpn=*/true});
#endif
    return failures;
}
