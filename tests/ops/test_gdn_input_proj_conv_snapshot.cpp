...
int run_fp8() {
    ...
    // Past one QPN pass (32 columns) the A16 projection runs a pass per 32 columns, and from 161
    // columns takes the CUTLASS route, whose weight staging the snapshot plan must reserve beside
    // its projected tile.
    failures += run_fp8_case(parent, 40, ops::LinearPolicy::A16Only, 41);
    failures += run_fp8_case(parent, 161, ops::LinearPolicy::A16Only, 162);
#endif
#ifndef NINFER_VOLTA_BUILD
