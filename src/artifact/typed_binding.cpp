...
// ---------------------------------------------------------------------------------------------
// Naming: v2 (flat, fused) -> v3 (logical, one binding per sub-tensor)
// ---------------------------------------------------------------------------------------------
...
// Expands a canonical v3 name into the list of v3 logical bindings that compose it.
...
// A v3 Binding must cover exactly one whole physical object to be exposed as a v2 tensor
// (the targets address whole objects, never slices).
...
// Resolves a v2 tensor name to the v3 physical object its leaves cover end to end.
...
std::string divisor_owner_name(std::string_view v2_name) {
    ...
    return owner; // gate_up, down, output, output_head: already the v2 weight name
}
...
// Resolves a v2 input divisor to the v3 `activation_input_divisor` auxiliary.
//
// Each v3 leaf of the consuming weight must have at least one Use carrying the auxiliary, and all
// of a leaf's Uses must name the same divisor object: a parameter read from several inputs (the
// output head, read from the text, MTP and DFlash2 hidden states) shares one divisor rather than
// having one per input. A fused v2 weight (gate_up) has one v3 leaf per sub-tensor, each with its
// own divisor object; the kernels consume a single divisor, so their FP32 words must be equal.
// The first leaf's object is returned.
...
ObjectHandle bind_raw_resource(Binder& binder, std::string_view name) {
    // v2: "frontend/<role>"; v3: each component carries its own resources
    // (tokenizer under "text", preprocessor under "vision").
