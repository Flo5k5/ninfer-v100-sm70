...
Bf16GdnNormGatingPlan bf16_gdn_norm_gating_resolve_plan(const Bf16GdnGatingProblem& problem) {
    ...
    // One plain-SIMT launch computes the norm, both control dots and h for the 27B geometry (the
    // Volta port had excluded it with the sm_80-only DFlash2 units, but it uses no sm_80
    // feature). On Volta it replaces rmsnorm + the SmallTSplit10 partial/reduce pair: three
    // latency-bound launches per GDN layer, 48 layers per verify step.
