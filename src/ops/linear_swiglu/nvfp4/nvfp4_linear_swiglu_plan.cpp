...
#ifdef NINFER_VOLTA_BUILD
// Both A16 QPN split entries below (the T=1 shortcut and the VoltaQpnSplit route) take FP16
// activations; every other route reads BF16.
bool nvfp4_linear_swiglu_fp16_activation_supported(const Weight& weight, LinearPolicy policy,
                                                   std::int32_t tokens) {
    return policy == LinearPolicy::A16Only &&
           nvfp4_linear_swiglu_qpn_split_supported(weight.k, tokens);
}
#endif
