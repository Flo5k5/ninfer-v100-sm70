...
// stream out of BF16 between updates; it is available on Volta builds for qwen3.8-27b/nvfp4 and
// qwen3.8-27b/nvfp4-full-a without DFlash, and the Engine rejects it anywhere else at construction.
...
enum class ContextLookupPolicy : std::uint8_t {
    ...
    Adaptive, // per-request suffix thresholds, split by tool-call region, tuned by outcomes
};
...
struct ContextLookupOptions {
    ...
    std::uint32_t max_proposal = kContextLookupMaximumProposal;
};
...
struct SpeculativeStats {
    ...
    // MTP rounds that verified a copied context continuation; a subset of `rounds`. Entry rounds
    // are the subset verified by the narrower adaptive entry tier.
    std::uint64_t lookup_rounds          = 0;
    std::uint64_t lookup_entry_rounds    = 0;
    std::uint64_t lookup_drafted_tokens  = 0;
    std::uint64_t lookup_accepted_tokens = 0;
    // Submit-to-completion wall time of MTP rounds with and without a copied continuation.
    double lookup_round_seconds = 0.0;
    double mtp_round_seconds    = 0.0;
};
